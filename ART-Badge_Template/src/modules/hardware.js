...
var keyfb_dcm_name = "btnfb"
var beep_period_dcm_name = "beepperiod"
var beep_duty_dcm_name = "beepduty"
var music_prev_dcm_name = "musicprev"
var music_next_dcm_name = "musicnext"
var music_duty_dcm_name = "musicduty";
var music_prog_dcm_name = "musicprog";
...
/* 按键反馈 */
function keyfb_enun(flag) {
    data_pool = dcmlib.Open(dcm_pool_name);
    data_pool.setItem(keyfb_dcm_name, flag);
    emqlib.send(emq_keyps_channel, ID_BTNFB);
    return true;
}
...
/* 播放音乐 */
function playmusic() {
    emqlib.send(emq_beep_channel, ID_MUSIC_PLAY);
    return true;
}

/* 暂停音乐 */
function stopmusic() {
    emqlib.send(emq_beep_channel, ID_MUSIC_SUSPEND);
    return true;
}

/* 上一首音乐 */
function play_prev_music(index) {
    data_pool = dcmlib.Open(dcm_pool_name);
    data_pool.setItem(music_prev_dcm_name, index);
    emqlib.send(emq_beep_channel, ID_MUSIC_PREV);
    return true;
}

/* 下一首音乐 */
function play_next_music(index) {
    data_pool = dcmlib.Open(dcm_pool_name);
    data_pool.setItem(music_next_dcm_name, index);
    emqlib.send(emq_beep_channel, ID_MUSIC_NEXT);
    return true;
}

/* 获取音乐时长 */
function getmusic_duration() {
    emqlib.send(emq_beep_channel, ID_MUSIC_DURT);
    data_pool = dcmlib.Open(dcm_pool_name);
    var info = data_pool.getItem(music_duty_dcm_name);
    if (info != null) {
        return info;
    }
    return null;
...
/* 获取音乐进度 */
function getmusic_progress() {
    emqlib.send(emq_beep_channel, ID_MUSIC_PROG);
    data_pool = dcmlib.Open(dcm_pool_name);
    var info = data_pool.getItem(music_prog_dcm_name);
    if (info != null) {
        return info;
    }
    return null;
...
module.exports = {
    ...
    getInfo: getInfo,
    keyfb_enun: keyfb_enun,
    openBeep: openBeep,
    closeBeep: closeBeep,
    openBeep: openBeep,
    playmusic: playmusic,
    stopmusic: stopmusic,
    play_prev_music: play_prev_music,
    play_next_music: play_next_music,
    getmusic_duration: getmusic_duration,
    getmusic_progress: getmusic_progress,
}